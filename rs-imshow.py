...
try:
    while True:
        ...
        canny_mask = cv2.Canny(gaussBlur_mask, 100, 200, apertureSize = 5, L2gradient=True)
        ...
        print(type(canny_mask))
        lines =	cv2.HoughLinesP(canny_mask, 1, np.pi / 180, 50, 50, 10)
            ...
            for line in lines:
                ...
                while(1):
                    ...
                    if nX2 < 0 or nY2 < 0 or nY2 >= height / 2 or canny_mask[nY2, nX2] == 0:
                        ...
                        break
                '''
                cv2.line(color_image,(x1,y1 + height // 2),(x2,y2 + height // 2),(0,0,255),2)
        cv2.imshow('win1', color_image)
        #cv2.imshow('win2', gray_color_image)
        #cv2.imshow('win3', gray_color_image2)
        #cv2.imshow('win4', hls_image)
        #cv2.imshow('win5', mask)
        #cv2.imshow('win6', gaussBlur_mask)
        cv2.imshow('win7', canny_mask)
        key = cv2.waitKey(1)
